...
const Node = ({
  ...
  const isOpen = openDropdowns.has(file.details.name);

  return (
    <li>
      <button onClick={() => toggleDropdown(file.details)}>
        {file.children.length > 0 && (isOpen ? <FaAngleDown /> : <FaAngleRight />)}
        <span>{file.details.name}</span>
      </button>
...
export default function FileTree({ files, onLastClicked }) {
  ...
  const buildFileTree = (fileArray: FileDetails[]): FileNode[] => {
    ...
    for (let file in map) {
      ...
      const mentions = extractMentions(
        map[file].details.content,
        temp.map((f) => f.name),
      ); // see if it mentions the names of other files in this file
      mentions.forEach((mention) => {
        // for each mention of another file
        if (
          map[mention] &&
          !map[file].children.includes(map[mention]) &&
          mention !== map[file].details.name
        ) {
  ...
  return (
    <div className="dropdown dropdown-end">
      <ul tabIndex={0} className="dropdown-item menu w-52 bg-base-100 p-2 shadow">
